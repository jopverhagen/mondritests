...
class Mondri(abc.ABC):
    ...
    @abc.abstractmethod
    def mondri_reverse_drift(self, x, score, t):
        pass
    ...
    def reverse_mondri_step_mean(self, x, score, t):
        return x - self.mondri_reverse_drift(x, score, t)

    def reverse_mondri_step(self, x, score, t):
        return x - self.mondri_reverse_drift(x, score, t) - self.dispersion(x, t)
    ...
    def reverse_mondri(self, xt, T=-1):
        T = self.T if T < 0 else T
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1))):
            score = self.score_fn(x, t)
            x = self.reverse_mondri_step(x, score, t)
...
class IRMondri(Mondri):
    ...
    def reverse_mondri(self, xt, T=-1, save_states=False, save_dir='mondri_state', **kwargs):
        T = self.T if T < 0 else T
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1))):
...
class DenoisingMondri(Mondri):
    ...
    def reverse_mondri(self, xt, x0=None, T=-1, save_states=False, save_dir='mondri_state'):
        T = self.T if T < 0 else T
        x = xt.clone()
        for t in tqdm(reversed(range(1, T + 1))):
            if x0 is not None:
