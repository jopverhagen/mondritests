...
class WaterDenoisingModel(BaseModel):
    def __init__(self, opt):
        super(WaterDenoisingModel, self).__init__(opt)
    ...
    def optimize_parameters(self, step, timesteps, mondri=None):
        mondri.set_mu(self.condition)
        ...
        # Get noise and score
        noise = mondri.noise_fn(self.state, timesteps.squeeze())
        score = mondri.get_score_from_noise(noise, timesteps)

        # Learning the maximum likelihood objective for state x_{t-1}
        xt_1_expection = mondri.reverse_mondri_step_mean(self.state, score, timesteps)
        xt_1_optimum = mondri.reverse_optimum_step(self.state, self.state_0, timesteps)
    ...
    def test(self, mondri=None, mode='posterior', save_states=False):
        mondri.set_mu(self.condition)

        self.model.eval()
        with torch.no_grad():
            if mode == 'mondri':
                self.output = mondri.reverse_mondri(self.state, save_states=save_states)
            elif mode == 'posterior':
                self.output = mondri.reverse_posterior(self.state, save_states=save_states)
